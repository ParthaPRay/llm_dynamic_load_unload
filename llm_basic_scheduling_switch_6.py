...
# curl -X POST http://localhost:5000/perform_task -H "Content-Type: application/json"   -d '{"task_type": "arithmetic", "model_name": "math_model", "prompt": "What is 2+2?"}'
...
MODEL_TIMEOUT = 60  # Unload models after 60 seconds of inactivity
DEBUG = True
...
def unload_idle_models():
    ...
    with model_lock:
        ...
        # Unload the current_model if it's idle for too long
        if current_model and (current_model not in model_last_used):
            debug_log(f"Unloading current active model '{current_model}' due to timeout.")
            payload = {"model": current_model, "keep_alive": 0}
            response = requests.post(f"{BASE_URL}/generate", json=payload)
            if response.status_code == 200:
                debug_log(f"Current model '{current_model}' unloaded successfully.")
                current_model = None
            else:
                debug_log(f"Failed to unload current model '{current_model}'. Error: {response.text}")
